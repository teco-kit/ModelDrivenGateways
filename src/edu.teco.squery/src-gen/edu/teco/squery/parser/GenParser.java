...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private sQueryLexer lexer;
	private sQueryParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public sQueryLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new sQueryLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public sQueryParser getParser() {
		if (parser == null) {
			parser = new sQueryParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "edu::teco::squery::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"edu::teco::squery::GenChecks","edu::teco::squery::Checks"};
