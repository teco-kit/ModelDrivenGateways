...
int lcm_serve_request (struct soap *soap);
...
INLINE struct service_register_cache_s *
epr_plugindata (struct ws4d_epr *epr)
{
  return (struct service_register_cache_s *) ws4d_epr_get_plugindata (epr,
                                                                     eprlist_lcm_id);
}
...
static int
service_register_fcreate (struct ws4d_epr *epr, struct ws4d_epr_plugin *plugin,
                          void *arg)
{
...
// -----------------
//
// set the given name to the cache
// element
/----------------------------------*/
int
set_cache_name (struct ws4d_epr *elem, char *name)
{
  struct service_register_cache_s *p_plugin = NULL;
  int size = 0;

  soap_assert (NULL, elem && name, LCM_ERR);
  ...
  memcpy (p_plugin->manifest.service_name, name, sizeof (char) * size);
...
/----------------------------------*/
int
set_cache_version (struct ws4d_epr *elem, char *version)
{
  struct service_register_cache_s *p_plugin = NULL;
  int size = 0;

  soap_assert (NULL, elem && version, LCM_ERR);
  ...
  memcpy (p_plugin->manifest.version, version, sizeof (char) * size);
...
/*----------------------------------
 ...
 // -----------------
 //
 // set the given wsdl loacation to
 // the cache element
 /----------------------------------*/
int
set_cache_wsdl (struct ws4d_epr *elem, char *wsdl)
{
  struct service_register_cache_s *p_plugin = NULL;
  int size = 0;

  soap_assert (NULL, elem && wsdl, LCM_ERR);
  ...
  memcpy (p_plugin->manifest.wsdl_name, wsdl, sizeof (char) * size);
...
/*----------------------------------
 ...
 // -----------------
 //
 // set the given xml loacation to
 // the cache element
 /----------------------------------*/
int
set_cache_xml (struct ws4d_epr *elem, char *xml)
{
  struct service_register_cache_s *p_plugin = NULL;
  int size = 0;

  soap_assert (NULL, elem && xml, LCM_ERR);
  ...
  memcpy (p_plugin->manifest.xml_name, xml, sizeof (char) * size);
...
/----------------------------------*/
int
compare (char *string, int index)
{
  unsigned int i = 0;
  unsigned int size = strlen (tag_list[index].manifest_tag);
  ...
  for (i = 0; i < size; i++)
    {
      if (string[i] != tag_list[index].manifest_tag[i])
        {
          /* not conform */
          return LCM_ERR;
        }
                ...
                struct _ns1__installResponse *ns1__installResponse)
{
  ...
  argumente[0] = "-x";
  argumente[1] = "-o";
  argumente[2] = string_tmp;
  argumente[3] = "-d";
  argumente[4] = get_cache_location (cache_elem);
  argumente[5] = NULL;

  do_unzip (5, argumente);
  ...
  switch (kind1 = fork ())
    ...
    case 0:
      sprintf (&string_tmp[0],
               "starting Service under new process with pid: %d",
               getpid ());
      err_print (&string_tmp[0]);
      sprintf (&string_tmp[0], "%s/%s", get_cache_location (cache_elem),
               get_cache_name (cache_elem));
                ...
                struct _ns1__ListAllResponse *ns1__ListAllResponse)
{
  ...
  ns1__ListAllResponse->services = malloc (index * sizeof (char *));
  ...
  ws4d_eprlist_foreach (elem, iter, &service_register)
  {
    if (elem)
      {
        err_print("-------------------------------------------------------------------------");
        sprintf (&string_tmp[0], "\t id      : %s", (get_cache_id (elem)) ? (get_cache_id (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t location: %s", (get_cache_location (elem)) ? (get_cache_location (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t status  : %d", get_cache_state (elem));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t valid   : %d", get_cache_valid (elem));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t name    : %s", (get_cache_name (elem)) ? (get_cache_name (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t wsdl    : %s", (get_cache_wsdl (elem)) ? (get_cache_wsdl (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t xml     : %s", (get_cache_xml (elem)) ? (get_cache_xml (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "\t version : %s", (get_cache_version (elem)) ? (get_cache_version (elem)) : ("NULL"));
        err_print (&string_tmp[0]);
        ...
        /* response */
        ns1__ListAllResponse->services[ns1__ListAllResponse->__sizeservices++] = get_cache_id (elem);
      }

    sprintf (&string_tmp[0], "array-size: %d", ns1__ListAllResponse->__sizeservices);
    err_print (&string_tmp[0]);

    for (index = 0; index < ns1__ListAllResponse->__sizeservices; index++)
      {
        sprintf (&string_tmp[0], "\t service %d: %s", index, ns1__ListAllResponse->services[index]);
        err_print (&string_tmp[0]);
      }
  }
                 ...
                 struct _ns1__getStateResponse *ns1__getStateResponse)
{
  ...
  err_print (&string_tmp[0]);
  sprintf (&string_tmp[0], "\t name    : %s", (get_cache_name (cache_elem)) ? (get_cache_name (cache_elem)) : ("NULL"));
  err_print (&string_tmp[0]);
  sprintf (&string_tmp[0], "\t wsdl    : %s", (get_cache_wsdl (cache_elem)) ? (get_cache_wsdl (cache_elem)) : ("NULL"));
  err_print (&string_tmp[0]);
  sprintf (&string_tmp[0], "\t xml     : %s", (get_cache_xml (cache_elem)) ? (get_cache_xml (cache_elem)) : ("NULL"));
  err_print (&string_tmp[0]);
  sprintf (&string_tmp[0], "\t version : %s", (get_cache_version (cache_elem)) ? (get_cache_version (cache_elem)) : ("NULL"));
  err_print (&string_tmp[0]);
               ...
               struct _ns1__updateResponse *ns1__updateResponse)
{
  ...
  char *argumente        [4];
  FILE *datei;
  void *so_handle = NULL;       /* handle to void function */
  int_intint_pf_t start_pf = NULL;      /* handle to start function */
  int_void_pf_t resolve_pf = NULL;      /* handle to resolve function */
   ...
   * //++++ */
  res = 0;
  sprintf (&string_tmp[0], "Attachment: Attachment size(%d)", ns1__update->Param.__size);
  err_print (&string_tmp[0]);
  sprintf (&string_tmp[0], "%s/service_%s.zip", get_cache_location (cache_elem), get_cache_version (cache_elem));

  if (writefile(&string_tmp[0], ns1__update->Param.__ptr, ns1__update->Param.__size) < 0)
    {
      /* can not save attachment --> error */
      err_print (error_list[LCM_ERR_SAVE_ATTACHMENT].string);
   ...
   * // unpack the zip-file
   * //++++ */
//  sprintf (&string_tmp[0], "cd %s\nunzip service_%s.zip", get_cache_location (cache_elem), get_cache_version (cache_elem));
//  system (&string_tmp[0]);
  ...
  if (readout_manifest
  ...
  else
    {
      ...
      err_print ("readout from manifest -->");
      sprintf (&string_tmp[0], "\tservice_name: %s", get_cache_name (cache_elem));
      err_print (&string_tmp[0]);
      sprintf (&string_tmp[0], "\twsdl_name   : %s", get_cache_wsdl (cache_elem));
      err_print (&string_tmp[0]);
      sprintf (&string_tmp[0], "\txml_name    : %s", get_cache_xml (cache_elem));
      err_print (&string_tmp[0]);
      sprintf (&string_tmp[0], "\tversion     : %s", get_cache_version (cache_elem));
      err_print (&string_tmp[0]);
    }
   ...
   * //++++ */
  if (check_version(get_cache_version (cache_elem), get_cache_version (save_cache)) != LCM_OK)
    {
  ...
  sprintf (&string_tmp[0], "using service-file: %s", get_cache_name (cache_elem));
  err_print (&string_tmp[0]);
  ...
  sprintf (&string_tmp[0], "%s/%s", get_cache_location (cache_elem), get_cache_xml (cache_elem));
  read_file (&string_tmp[0], xml);
  soap_init (&parser);
  soap_set_namespaces (&parser, lcm_namespaces);
  soap_begin_deser (&parser, xml, strlen (xml), lcm_namespaces);
  soap_default__wsm__Metadata (&parser, &Metadata);
  if (soap_get__wsm__Metadata (&parser, &Metadata, "wsm:Metadata", "") == NULL)
    {
      err_print (error_list[LCM_ERR_METADATA].string);
  ...
  /* start new process */
    switch (kind1 = fork ())
      ...
      case 0:
        sprintf (&string_tmp[0], "starting Service under new process with pid: %d", getpid ());
        err_print (&string_tmp[0]);
        sprintf (&string_tmp[0], "%s/%s", get_cache_location (cache_elem), get_cache_name (cache_elem));
        /* set the authorization */
  ...
  sprintf (&string_tmp[0], "%s/%s", get_cache_location (cache_elem), get_cache_xml (cache_elem));
  /* xml-file-to */
  sprintf (&string_to[0], "%s/%s.xml", hosting_xml_path, get_cache_id (cache_elem));
  /* copy file */
