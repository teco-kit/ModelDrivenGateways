...
enum cmdloop_cmds
{
  PRTHELP = 0, INSTALL, UNINSTALL, START, STOP, RESOLVE, STATE, ALL, UPDATE, EXITCMD
};
...
unsigned short OldCRC(unsigned short StartCRC,const void *Addr,unsigned int Size)
{
  unsigned char *Data=(unsigned char *)Addr;
  unsigned int i = 0;
...
//
//
void getState( struct _ns1__getStateResponse *Response, char *uuid )
{
...
*/    
	...
	query.Param.__size = readfile("/home/schorsch/workspace/life-cycle-manager/se.zip", query.Param.__ptr);
...
update (int argc, char **argv)
{
	...
	query.Param.__size = readfile("/home/schorsch/workspace/life-cycle-manager/se_update.zip", query.Param.__ptr);
...
//  wsa_eprlist_elem_set_Addrs (service, wsa_eprlist_elem_get_Addrs (service));
  ...
  ws4d_qnamelist_addstring("\"http://www.uni-rostock.de/schemas/hostingService\":hostingService", &type_list, &alist);

//TODO Win32  service = wsa_eprlist_alloc_elem (1, &alist);
  if (dpws_probe( &dpws, &type_list, NULL, 6000, 100, NULL, 1, service) == SOAP_OK)
  {
...
//TODO Win32  type = wsa_eprlist_alloc_elem (1, &alist);
  ...
  ws4d_qnamelist_addstring("\"http://www.ws4d.org/services/life-cycle-manager\":life-cycle-manager", &service_list, &alist);
  
  if (dpws_find_services (&dpws, service, &service_list,
                         10000, 1, type) == SOAP_OK)
    {
