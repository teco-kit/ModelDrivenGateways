  ...
  if (s)
    {
      ...
      /* only proceed if length is in valid range */
      if (len < 7 || len > 15)
        return res;

      /* NOTE: Refering to spec an ipv4 address has the following syntax:
       * Ipv4address = 1*digit"."1*digit"."1*digit"."1*digit
       * => 300.1.1.1 is valid !?
       */

      for (; s[cur] != '\0' && segments <= 3; cur++)
        {
          if (s[cur] == '.' && (cur - last) > (segments == 0 ? 0 : 1))
            {
              /* handle end of segment */
              segments += 1;
              last = cur;
            }
          else if (cur - last > (segments == 0 ? 2 : 3))
            {
              return res;       /* validation failed */
            }
          else
          ...
          if (uri[cur + i] == '@')
            {
              ...
              cur += i + 1;     // skip the userinfo and the @
            }
        }
