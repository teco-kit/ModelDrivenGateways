...
eprflist_fdone (struct ws4d_abs_eprlist *list)
{
  ...
  ws4d_assert (eprflist_checkid (list) && data && list->fclear,);
...
{
  struct ws4d_eprlist_data *data;
  register struct ws4d_epr *alloced, *next;
  int is_alloced = 0;
...
ws4d_eprflist_init (struct ws4d_abs_eprlist *head, void *arg)
{
  ...
  head->fadd = eprflist_fadd;
  head->falloc = eprflist_falloc;
  head->fclear = eprflist_fclear;
  head->fdone = eprflist_fdone;
  head->ffree = eprflist_ffree;
  head->fget_first = eprflist_fget_first;
  head->fget_next = eprflist_fget_next;
  head->fget_byAddr = eprflist_get_byAddr;

#ifdef WITH_MUTEXES
  head->flock = eprflist_flock;
  head->funlock = eprflist_funlock;
#endif
