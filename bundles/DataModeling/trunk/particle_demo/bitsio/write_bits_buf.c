...
ssize_t write_bits(u_char *bits, int bits_len)
{
   ...
   //timeUntilSlot1 = RFMicroSecondsUntilNextSlot();
   ...
   if (writer.currBits > 0)
   {
	 ...
	 u_char cur_byte;
	 cur_byte=bits[i];
         a_byte = (cur_byte >> writer.currBits);
	 a_byte |= (writer.lastByte);

         writer.lastByte = LOW_N_BITS_SHIFT(cur_byte, writer.currBits);
   ...
   //timeUntilSlot2 = RFMicroSecondsUntilNextSlot();
   //timeSpent += (timeUntilSlot1 - timeUntilSlot2);
      return ret;
   }
   else
   {
