...
ws4d_serviceep_gettypestr (struct ws4d_epr *epr, ws4d_alloc_list * alist)
{
  ...
  /* return copy of qname list string */
  return ws4d_qnamelist_tostring (&plugin->types, alist);
}
...
char *
ws4d_serviceep_copyid (struct ws4d_epr *epr, ws4d_alloc_list * alist)
{
  struct ws4d_serviceep_plugin_s *plugin = ws4d_serviceep_plugindata (epr);
  ...
  return ws4d_strdup (plugin->id, alist);
}
