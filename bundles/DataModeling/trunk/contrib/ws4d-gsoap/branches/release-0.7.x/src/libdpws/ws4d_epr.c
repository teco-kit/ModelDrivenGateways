...
ws4d_epr_copy (struct ws4d_epr *dst, struct ws4d_epr *src)
{
  ...
  ws4d_list_foreach (plugin, next, &src->plugins, struct ws4d_epr_plugin, list)
  {
...
int
ws4d_epr_reset (struct ws4d_epr *epr)
{
  /* test parameters */
  ws4d_assert (epr, WS4D_ERR);
...
ws4d_epr_remove_plugins (struct ws4d_epr *epr)
{
  ...
  ws4d_list_foreach (plugin, next, &epr->plugins, struct ws4d_epr_plugin, list)
  {
    if (plugin)
      {
        if (plugin->cb && plugin->cb->fdelete)
