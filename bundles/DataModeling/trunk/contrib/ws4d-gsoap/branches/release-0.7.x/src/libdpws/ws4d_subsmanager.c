...
int ws4d_subsm_done(struct ws4d_subsmanager *subsm)
{
  ...
  ws4d_list_foreach(cur, next, &subsm->subs_list, struct ws4d_subscription, list)
    {
...
struct ws4d_subscription *ws4d_subsm_getsubs(struct ws4d_subsmanager *subsm, const char *id)
{
...
struct ws4d_subscription *ws4d_subsm_allocsubs(struct ws4d_subsmanager *subsm)
  {
    int err;
    ...
    subs = ws4d_malloc_alist(sizeof(struct ws4d_subscription), ws4d_subsm_get_alist(subsm));
    if (!subs)
      return NULL;
...
int ws4d_subsm_freesubs(struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
...
int ws4d_subsm_regsubs(struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
  /* test parameter */
  ws4d_assert (subsm && subs && subs->delivery_mode && subs->filter_mode, WS4D_ERR);
...
int ws4d_subsm_expiresubs(struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
  ws4d_assert (subsm && subs, WS4D_ERR);
                                   ...
                                   ws4d_cleanDelivery_cb fcleanDelivery)
{
  ...
  deliveryType =
    ws4d_malloc_alist (sizeof (struct ws4d_event_delivery), ws4d_subsm_get_alist(subsm));
  if (!deliveryType)
    return WS4D_EOM;
                                     ...
                                     const char *Mode)
{
  ...
  ws4d_list_foreach (delivery, next, &subsm->supp_delivery_list, struct ws4d_event_delivery, list)
  {
                                 ...
                                 ws4d_cleanFilter_cb fcleanFilter)
{
  ...
  filterType =
    ws4d_malloc_alist (sizeof (struct ws4d_event_filter), ws4d_subsm_get_alist(subsm));
  if (!filterType)
    return WS4D_EOM;
                                   ...
                                   const char *Type)
{
  ...
  ws4d_list_foreach (filter, next, &subsm->supp_filter_list, struct ws4d_event_filter, list)
  {
    if (filter->Type == Type)
      {
...
ws4d_subsm_is_delivery (struct ws4d_subscription *subs, const char *Mode)
{
  ...
  if ((Mode == subs->delivery_mode->DeliveryMode) || (!strcmp (Mode, subs->delivery_mode->DeliveryMode)))
    return 1;
  else
    return 0;
}
...
ws4d_subsm_is_filter (struct ws4d_subscription *subs, const char *Type)
{
  ...
  if ((Type == subs->filter_mode->FilterType) || (!strcmp (Type, subs->filter_mode->FilterType)))
    return 1;
  else
    return 0;
}
...
int
ws4d_subsm_clean_delivery (struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
...
int
ws4d_subsm_clean_filter(struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
...
int
ws4d_subsm_check_subs (struct ws4d_subsmanager *subsm, struct ws4d_subscription *subs)
{
  int res = 0;
  if (subs)
    {
...
ws4d_subsm_check_allsubs (struct ws4d_subsmanager *subsm)
{
  ...
  ws4d_list_foreach (subs, next, &subsm->subs_list, struct ws4d_subscription, list)
  {
