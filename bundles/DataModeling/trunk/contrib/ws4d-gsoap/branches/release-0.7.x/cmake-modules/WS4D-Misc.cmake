...
MACRO(DPWS_METADATA_GENERATE metadata_file prefix destination)
	IF(DPWS_XSLTPROC_BINARY)
		...
		ADD_CUSTOM_COMMAND(OUTPUT ${dpws_metadata_generated_files}
			COMMAND ${DPWS_XSLTPROC_BINARY} --stringparam nsprefix ${prefix} ${DPWS_METADATATOC} ${CMAKE_CURRENT_SOURCE_DIR}/${metadata_file} > ${destination}/${prefix}_metadata.c
			COMMAND ${DPWS_XSLTPROC_BINARY} --stringparam nsprefix ${prefix} ${DPWS_METADATATOH} ${CMAKE_CURRENT_SOURCE_DIR}/${metadata_file} > ${destination}/${prefix}_metadata.h
			DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/${metadata_file})
	ENDIF(DPWS_XSLTPROC_BINARY)
	...
	FOREACH (it ${ARGN})
        ...
        IF(CYGWIN)
            ADD_CUSTOM_COMMAND(OUTPUT ${outfile}
                ...
                COMMAND ${GNU_SED_BINARY} ARGS -i -e '/^..gsoapopt/d' ${outfile}
                DEPENDS ${it} ${typemap})
            ELSE(CYGWIN)
            ADD_CUSTOM_COMMAND(OUTPUT ${outfile}
