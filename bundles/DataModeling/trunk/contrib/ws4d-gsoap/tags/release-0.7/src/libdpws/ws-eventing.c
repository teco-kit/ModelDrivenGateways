...
int
wse_subsm_process_deliveryMode (struct ws4d_subsmanager *subsm, struct soap *src, struct ws4d_subscription *subs, struct wse__DeliveryType *Delivery)
{
  register struct ws4d_event_delivery *deliveryType, *next;

  if (!subsm || !src || !Delivery || !Delivery->Mode)
    return wse_error (src,
                      wse__FaultSubcodeValues__wse__DeliveryModeRequestedUnavailable);

  ws4d_list_foreach (deliveryType, next, &subsm->supp_delivery_list, struct ws4d_event_delivery, list)
  {
    if (!strcmp (deliveryType->Mode, Delivery->Mode))
      {
        if (deliveryType->fprocessDelivery)
          {
            int err;

            subs->delivery_mode = ws4d_malloc_alist(sizeof(struct ws4d_delivery_type), ws4d_subsm_get_alist(subsm));
            if (!subs->delivery_mode)
              {
                return SOAP_ERR;
              }

            err = deliveryType->fprocessDelivery (subs, Delivery);
            if (err == WS4D_OK)
              return SOAP_OK;
            else
              return SOAP_ERR;
...
int
wse_subsm_process_filterType (struct ws4d_subsmanager *subsm, struct soap *src, struct ws4d_subscription *subs, struct wse__FilterType *Filter)
{
  register struct ws4d_event_filter *filterType, *next;

  if (!subsm || !src || !Filter || !Filter->Dialect)
    return wse_error (src,
                      wse__FaultSubcodeValues__wse__FilteringRequestedUnavailable);

  ws4d_list_foreach (filterType, next, &subsm->supp_filter_list, struct ws4d_event_filter, list)
  {
    if (!strcmp (filterType->Type, Filter->Dialect))
      {
        if (filterType->fprocessFilterType)
          {
            int err;

            subs->filter_mode = ws4d_malloc_alist(sizeof(struct ws4d_filter_type), ws4d_subsm_get_alist(subsm));
            if (!subs->filter_mode)
              {
                return SOAP_ERR;
              }

            err = filterType->fprocessFilterType (subs, Filter);
            if (err == WS4D_OK)
              return SOAP_OK;
            else
              return SOAP_ERR;
...
int
wse_subsm_end_subs (struct soap *soap, char *MsgId, struct ws4d_subscription *subs, const char *status, char *reason)
{
  int err;
  struct wse_subsm_plugin_data *data = wse_subsm_get_plugindata (soap);
  ...
  if (subs->endto)
    {
      ...
      SubscriptionEnd.SubscriptionManager.Address = ws4d_subsm_getaddr(&data->subsm);
      if (status)
...
{
  struct wse_evsrc_plugin_data *ddata = dst->data;
  struct wse_evsrc_plugin_data *sdata = src->data;
  ddata->subsm = sdata->subsm;
  return SOAP_OK;
}
...
wse_evsrc_plugin_init (struct soap *soap, struct soap_plugin *p, void *arg)
{
  ...
  p->data =
    (void *) SOAP_MALLOC (soap, sizeof (struct wse_evsrc_plugin_data));
...
int
wse_process_subscribe_req (struct soap *soap, struct _wse__Subscribe *req, struct ws4d_subscription **subs)
{
  int err = 0;
  ws4d_time expires;
  ...
  if ((!req->Delivery || !req->Delivery->Mode)
      || (!req->Filter || !req->Filter->Dialect))
    return SOAP_OK;
  ...
  err = wse_subsm_process_deliveryMode (&subsm_data->subsm, soap, new_subs, req->Delivery);
  if (err)
    {
  ...
  /* Process Filter Type */
  err = wse_subsm_process_filterType (&subsm_data->subsm, soap, new_subs, req->Filter);
  if (err)
    {
      ws4d_subsm_unlock(&subsm_data->subsm);
      return err;
                           ...
                           struct _wse__SubscribeResponse *res)
{
  ...
  ws4d_s_to_dur (ws4d_subs_get_expires(subs) - ws4d_systime_s (), &res->Expires);
...
wse_process_renew_req (struct soap *soap,
                       struct _wse__Renew *req, struct ws4d_subscription **subs_p)
{
  ws4d_time expires;
  struct ws4d_subscription *subs = NULL;
  struct wse_subsm_plugin_data *data = wse_subsm_get_plugindata (soap);

  soap_assert(soap, data && soap->header && soap->header->wse__Identifier, SOAP_ERR)
...
int
wse_process_renew_res (struct soap *soap, char *to, struct ws4d_subscription *subs, struct _wse__RenewResponse *res)
{
...
wse_process_getstatus_req (struct soap *soap,
                           struct _wse__GetStatus
                           *req, struct ws4d_subscription **subs_p)
{
  struct ws4d_subscription *subs = NULL;
  struct wse_subsm_plugin_data *data = wse_subsm_get_plugindata (soap);
...
int
wse_process_unsubscribe (struct soap *soap, struct _wse__Unsubscribe *__req)
{
  struct ws4d_subscription *subs = NULL;
  struct wse_subsm_plugin_data *data = wse_subsm_get_plugindata (soap);
...
wse_subscribe (struct soap *soap, const char *MsgId, const char *serviceAddress, const char *endToAddress, struct wse__DeliveryType *delivery, struct ws4d_dur *expires, struct wse__FilterType *filter)
{
...
int
wse_subs_renew (struct soap *soap, const char *MsgId, const char *subs_man, const char *id, struct ws4d_dur *expires)
{
...
struct ws4d_dur *
wse_subs_get_status (struct soap *soap, const char *MsgId, const char *subs_man, const char *id)
{
...
int
wse_subs_unsubscribe (struct soap *soap, const char *MsgId, const char *subs_man, const char *id)
{
  ...
  return soap_call___wse__Unsubscribe (soap,
                                       subs_man, NULL, &req, NULL);
}
                        ...
                        const struct ws4d_dur *a, const char *type)
{
  if (!soap_element_begin_out
    ...
    {
      int ret = 0;
      char buf[XSD_DURATION_LEN];
      memset (buf, 0, XSD_DURATION_LEN);
      SNPRINTF (buf, XSD_DURATION_LEN,
                "P%dY%dM%dDT%dH%dM%dS", a->years,
...
/*  if (*soap->type
  ...
  a =
    (struct ws4d_dur *) soap_id_enter (soap, soap->id, a,
                                  ...
                                  sizeof (struct ws4d_dur), 0, NULL, NULL, NULL);
  if (*soap->href)
    a =
      (struct ws4d_dur *) soap_id_forward (soap, soap->href,
                                      a, 0,
                                      SOAP_TYPE_xsd__duration,
                                      0, sizeof (struct ws4d_dur), 0, NULL);
  else if (a)
    {
      ...
      if (s)
        {
          ...
          period = strchr (s, 'P') + 1;
          time = strchr (s, 'T') + 1;
          sscanf (period, "%dY%dM%dD", &a->years, &a->months, &a->days);
          sscanf (time, "%dH%dM%dS", &a->hours, &a->minutes, &a->seconds);
        }
    }
