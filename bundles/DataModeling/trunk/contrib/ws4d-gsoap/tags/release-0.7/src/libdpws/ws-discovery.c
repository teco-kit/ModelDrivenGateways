...
/**
 * logical address of discover service
 */
                       ...
                       const char *group, short int port, int backlog)
{
  ...
  /* add to ws discovery mutlicast group */
...
static int wsd_connect (struct soap *soap, const char *endpoint,
...
static int wsd_mca_plugin_copy (struct soap *soap, struct soap_plugin *dst,
...
/**
 * Function to copy discovery unicast plugin data.
 * Function is called by soap_copy().
 *
 * @param soap soap handle
 * @param dst pointer to soap plugin destination structure
 * @param src pointer to soap plugin source structure
 *
 * returns SOAP_OK on success, SOAP_ERR otherwise
 */

static int
wsd_mca_plugin_copy (struct soap *soap, struct soap_plugin *dst,
                     struct soap_plugin *src)
{
  return SOAP_OK;
}
                         ...
                         struct wsd_mca_hooks_t *hooks)
{
  ...
  err = wsd_bind_multicastudp (soap, netdev, WSD_MC_IPADDR, WSD_MC_UDPPORT,
                               backlog);
  if (err != SOAP_OK)
    return err;
                              ...
                              ws4d_alloc_list * alist)
{
  ...
  result = soap_extend_namespaces ((struct Namespace *) namespaces,
                           ...
                           ws4d_alloc_list * alist)
{
  ...
  ns = wsd_target_extend_namespaces (target, namespaces, alist);

  if (ns)
    {
      soap_set_namespaces (soap, ns);
      return SOAP_OK;
    }
  else
    {
                ...
                ws4d_alloc_list * alist)
{
  ...
  /* Prepare SOAP Header */
  wsa_header_gen_oneway (soap, MsgId, WSD_EP,
                      ...
                      struct wsd__ProbeMatchesType *wsd__ProbeMatches)
{
  ...
  ws4d_eprlist_foreach (target, next, matches)
  {
    ...
    probeMatch->wsa__EndpointReference.Address = (char *) ws4d_epr_get_Addrs (target);
                             ...
                             ws4d_alloc_list * alist)
{
  ...
  ws4d_eprlist_foreach (target, next, matches)
  {
    ...
    wsd_target_set_namespaces (target, soap, namespaces, alist);
  }


  /* Prepare SOAP Header */
  wsa_header_gen_response (soap, MsgId, (char *) wsa_anonymousURI,
                   ...
                   struct ws4d_abs_eprlist *matching_targets)
{
  ...
  err =
    ws4d_targetservice_getmatches (mca_data->targetservice, scopes, types,
                                   matching_targets);
  if (err)
    return SOAP_ERR;
                         ...
                         struct soap *req, ws4d_alloc_list * alist)
{
  ...
  /* Prepare SOAP Header */
  wsa_header_gen_response (soap, MsgId, WSD_EP,
  ...
  soap_default_wsa__EndpointReferenceType (soap, &resolveMatch.
                                           wsa__EndpointReference);
...
static int
wsd_uni_plugin_copy (struct soap *soap, struct soap_plugin *dst,
                     struct soap_plugin *src)
{
  return SOAP_OK;
}
                         ...
                         struct wsd_dis_hooks_t *dis_hooks)
{
  ...
  /* test parameters */
  if (!mca || !netdev || uni_data || mca_data)
    return SOAP_ERR;
...
wsd_process_hello (struct soap *soap, struct wsd__HelloType *req)
{
  ...
  if (mca_data->dis_hooks && mca_data->dis_hooks->NewTarget)
    {
      ...
      if (req->wsa__EndpointReference.Address)
        {
          ws4d_epr_set_Addrs (target, req->wsa__EndpointReference.Address);
        }
      /* TODO: copy more epr data here */
  ...
  if (mca_data->target_cache)
    {
      int target_state = 0;
      char *Address = req->wsa__EndpointReference.Address;
  ...
  if (mca_data->target_cache)
    {
      ...
      ws4d_tc_invalidate_epr (mca_data->target_cache,
                              req->wsa__EndpointReference.Address);
                 ...
                 ws4d_alloc_list * alist)
{
  ...
  /* Prepare SOAP Header */
  wsa_header_gen_request (soap, MsgId, WSD_EP,
                            ...
                            struct wsd__ResolveMatchesType *req)
{
  ...
  /* test message */
  if (req->ResolveMatch && req->ResolveMatch->wsa__EndpointReference.Address
      && req->ResolveMatch->wsd__XAddrs)
    {
      char *Address = NULL;
      ...
      if ((target_state == WS4D_TARGETCACHE_NEW) || (target_state
                                                     ...
                                                     WS4D_TARGETCACHE_VALID))
        {
          ...
          /* update XAddrs */
          if (req->ResolveMatch->wsd__XAddrs)
            {
              ws4d_tc_update_XAddrs (uni_data->target_cache, Address,
                                     req->ResolveMatch->wsd__XAddrs);
            }
