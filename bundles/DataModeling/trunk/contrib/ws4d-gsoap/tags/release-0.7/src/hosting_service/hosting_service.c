...
char *host = NULL;
char *uuid = NULL;
char *metadata = HOSTING_SERVICE_CONFIG_FILE;
char *location = HOSTING_SERVICE_CONFIG_DIR;
...
int
setup_hostingservice (struct dpws_s *device,
                      struct soap *handle, char *xml_file, char *uuid)
{
  char *xml;
  int i;
  struct soap parser;
  ...
  for (i = 0; i < Metadata.__sizeMetadataSection; i++)
    {
      ...
      if (MetadataSection && MetadataSection->Dialect
          && !strcmp (MetadataSection->Dialect, DPWS_MEX_RELATIONSHIP))
        {
          ...
          if (dpws_add_hosting_service
              (device, handle, 0, Relation->wsdp__Host->wsdp__ServiceId, uuid,
               100))
            {
      ...
      if (MetadataSection && MetadataSection->Dialect
          && !strcmp (MetadataSection->Dialect, DPWS_MEX_MODEL))
        {
          ...
          dpws_copy_ThisModel (model, ThisModel, &device->alloc_list);
      ...
      if (MetadataSection && MetadataSection->Dialect
          && !strcmp (MetadataSection->Dialect, DPWS_MEX_DEVICE))
        {
          ...
          dpws_copy_ThisDevice (_device, ThisDevice, &device->alloc_list);
...
int
setup_hostedservice (struct dpws_s *device, struct xml_file *xml_file)
{
  char *xml;
  int i, j;
  struct soap parser;
  ...
  ws4d_assert (device && xml_file, SOAP_ERR);
          ...
          for (j = 0; j < Relation->__sizeHosted; j++)
            {
               ...
               */
              service_s = dpws_service_init (device, Hosted->wsdp__ServiceId);
              if (service_s == NULL)
                {
                  printf ("\nCan't init services\n");
                  dpws_done (device);
...
int
shutdown_hostedservice (struct dpws_s *device, struct xml_file *xml_file)
{
      ...
      if (!found)
        {
          ...
          /* parse file */
          setup_hostedservice (&device, file);
    ...
    if (elem && elem->found == 0)
      {
        ...
        shutdown_hostedservice (&device, elem);
...
main (int argc, char **argv)
{
  ...
  setup_hostingservice (&device, &service, metadata, uuid);
