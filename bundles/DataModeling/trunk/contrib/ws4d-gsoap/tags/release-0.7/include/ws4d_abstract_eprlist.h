                                 ...
                                 head, void *arg), void *arg)
{
  ...
  ws4d_assert (head->fdone && head->falloc && head->ffree
               && head->fadd && head->fremove && head->fclear
               && head->fisempty && head->fget_first && head->fget_next
               && head->fget_byAddr, WS4D_ERR);
...
{
  ws4d_assert (head && head->falloc, NULL);

  return head->falloc (head);
}
...
{
  ws4d_assert (head && head->ffree && epr, WS4D_ERR);

  return head->ffree (head, epr);
}
...
{
  ws4d_assert (head && head->fadd && head->fget_byAddr && epr, WS4D_ERR);

  return head->fadd (head, epr);
}
...
{
  ws4d_assert (head && head->fremove && epr, WS4D_ERR);

  return head->fremove (head, epr);
}
...
{
  ws4d_assert (head && head->fisempty, WS4D_ERR);

  return head->fisempty (head);
}
...
{
  ws4d_assert (head && head->fclear, WS4D_ERR);

  return head->fclear (head);
}
...
{
  ws4d_assert (head && head->fget_first, NULL);

  return head->fget_first (head);
}
...
{
  ws4d_assert (head && head->fget_next, NULL);

  return head->fget_next (head, epr);
}
...
{
  ws4d_assert (head && head->fget_byAddr && Addr, NULL);

  return head->fget_byAddr (head, Addr);
}
